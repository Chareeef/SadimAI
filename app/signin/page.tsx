...
export default function SignIn() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#020a06]/15">
      ...
      <Header />
      <main className="relative z-10 px-5 py-12 sm:px-8 sm:py-16 lg:py-20">
        <motion.div
          initial={{ opacity: 0, y: 24 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.65, ease: "easeOut" }}
          className="relative mx-auto grid w-full max-w-6xl overflow-hidden rounded-[28px] border border-emerald-100/[0.09] bg-[#07140f]/90 shadow-[0_36px_100px_rgba(0,20,12,0.32)] backdrop-blur-3xl lg:min-h-[650px] lg:grid-cols-[1.08fr_0.92fr]"
        >
          <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_12%_0%,rgba(52,211,153,0.10),transparent_42%)]" />
          <section className="relative hidden overflow-hidden border-r border-white/[0.07] p-12 lg:flex lg:flex-col lg:justify-between">
            <div className="relative">
              ...
              <p className="mb-6 mt-6 max-w-lg text-base leading-7 text-white/80">
            ...
            <div className="relative grid gap-4">
              {benefits.map((benefit) => (
                <div
                  key={benefit.title}
                  className="flex items-start gap-4 rounded-2xl border border-emerald-100/[0.08] bg-[#0a1b14]/95 p-4"
                >
          ...
          <section className="relative flex items-center justify-center px-5 py-14 sm:px-12 lg:px-14">
            <div className="w-full max-w-sm">
              ...
              <button
                ...
                disabled={isLoading}
                className="mt-9 flex min-h-13 w-full items-center justify-center gap-3 rounded-2xl bg-white px-5 py-3.5 text-sm font-semibold text-[#04110b] shadow-[0_14px_40px_rgba(0,0,0,0.25)] transition-all hover:-translate-y-0.5 hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300 disabled:translate-y-0 disabled:opacity-70"
              >
                {isLoading ? (
              ...
              <div className="grid grid-cols-3 gap-2 text-center">
                {[
                  ...
                  <div
                    key={label}
                    className="rounded-xl border border-emerald-100/[0.08] bg-[#0a1b14]/95 px-2 py-3"
                  >
                    <Icon icon={icon} className="mx-auto size-4 text-emerald-300/70" />
                    <p className="mt-2 text-[10px] font-medium leading-4 text-white/90">{label}</p>
                  </div>
                ))}
              </div>

              <p className="mt-8 text-center text-[11px] leading-5 text-white/75">
